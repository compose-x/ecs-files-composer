...
class CertbotAwsStoreCertificate(BaseModel):
    ...
    certificates_registry_table_name: Optional[str] = Field(
        None, description="Override the top level certificates_registry_table_name"
    )
...
class CertbotStore(BaseModel):
    certificates_registry_table_name: Optional[str] = Field(
        "certbot-registry", description="default table name to query for certificates."
    )
    certificates_registry_table_region: Optional[str] = Field(
        None,
        description="region to use for the certificates_registry_table_name. Defaults to current region found in profiles chain.",
    )
    certificates: Optional[Dict[str, CertbotAwsStoreCertificate]] = None
...
class X509CertDef(BaseModel):
    ...
    jks_config: Optional[JksConfig] = Field(None, alias="jksConfig")
...
class Model(BaseModel):
    ...
    certbot_store: Optional[CertbotStore] = None
    iam_override: Optional[IamOverrideDef] = Field(None, alias="IamOverride")
